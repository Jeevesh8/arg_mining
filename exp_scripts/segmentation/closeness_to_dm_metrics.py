...
from mask_comp_side import remove_comps

metric = load_metric('seqeval')
metric_near_dm = load_metric("seqeval")
metric_far_dm = load_metric("seqeval")
...
def evaluate(dataset, metric):
    ...
    with torch.no_grad():
        for tokenized_threads, masked_threads, comp_type_labels in get_comment_wise_dataset(dataset):
            ...
            masked_threads = [ [token_id for token_id in masked_token_ids[:lengths[i]]]
                              for i, masked_token_ids in enumerate(masked_threads.cpu().tolist())
                             ]
            
            near_dm_preds = remove_comps(preds, masked_threads, tokenizer.mask_token_id, 5, True)
            near_dm_refs = remove_comps(refs, masked_threads, tokenizer.mask_token_id, 5, True)
            far_dm_preds = remove_comps(preds, masked_threads, tokenizer.mask_token_id, 5, False)
            far_dm_refs = remove_comps(refs, masked_threads, tokenizer.mask_token_id, 5, False)
            
            metric_near_dm.add_batch(predictions=near_dm_preds,
                                     references=near_dm_refs,)
            metric_far_dm.add_batch(predictions=far_dm_preds,
                                    references=fard_dm_refs)
            metric.add_batch(predictions=preds, 
                            references=refs,)
                            #tokenized_threads=tokenized_threads.cpu().tolist())
        
    print("\t\t\t\t", metric.compute())
    print("\t\t\t\tNear DM Metrics:", metric_near_dm.compute())
    print("\t\t\t\tFar DM Metrics:", metric_far_dm.compute())
