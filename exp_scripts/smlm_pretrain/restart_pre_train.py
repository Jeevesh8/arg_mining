...
pt_location = "4epoch_complete/"

corpus = Corpus(filename=download("winning-args-corpus"))

tokenizer = LongformerTokenizer.from_pretrained(os.path.join(pt_location, 'tokenizer'))

transformer_model = LongformerForMaskedLM.from_pretrained(os.path.join(pt_location, 'model')).to(device)
...
def train(accumulate_over=3, save_ckpt_iters=20000, epoch_no=0):
...
def eval():
    with torch.no_grad():
        correct, total = 0, 0
        for masked_encoding, label_encoding in batch_generator(tokenized_ids_generator, 99, 100, batch_size=4096):
            ...
            correct += torch.sum(correct_predictions*masked_positions).cpu().item()
            total += torch.sum(masked_positions).cpu().item()
...
restart_from = 4
for i in range(restart_from, n_epochs):
    print(f"--------------Restarting: Beginning EPOCH {i+1}-------------")
    eval()
    train(epoch_no=i)
